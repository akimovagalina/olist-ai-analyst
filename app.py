        ...
        with st.status("🕵️‍♂️ ИИ-аналитик изучает хранилище данных маркетплейса...", expanded=True) as status:
            try:
                ...
                analyst_system_prompt = (
                    ...
                    "   - 📊 2. ПОДРОБНЫЕ ЦИФРЫ И ФАКТЫ: Проанализируй лидеров и аутсайдеров из таблицы. Назови конкретные категории/штаты и их точные метрики.\n"
                ...
                # Переключаем отчет на скоростную модель с ОГРОМНЫМ буфером вывода в 2000 токенов
                report_response = completion(
                    ...
                    temperature=0.3,
                    max_tokens=2000  # <-- РАСШИРИЛИ БУФЕР ВЫВОДА ДО МАКСИМУМА
                )
